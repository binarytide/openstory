...
beforeEach(async () => {
  projectRoot = await mkdtemp(join(tmpdir(), "openbook-init-"));
  writes = [];
...
const cases: FrameworkExpectation[] = [
  {
    ...
    previewMustContain: `from "openbook/react"`,
    viteMustContain: `from "@vitejs/plugin-react"`,
  },
  {
    framework: "solid",
    previewFile: "preview.tsx",
    previewMustContain: `from "openbook/solid"`,
    viteMustContain: `from "vite-plugin-solid"`,
  },
  {
    framework: "vue",
    previewFile: "preview.ts",
    previewMustContain: `from "openbook/vue"`,
    viteMustContain: `from "@vitejs/plugin-vue"`,
  },
  {
    framework: "svelte",
    previewFile: "preview.ts",
    previewMustContain: `from "openbook/svelte"`,
...
describe("runInit", () => {
  for (const expectation of cases) {
    it(`scaffolds preview + vite.config for ${expectation.framework}`, async () => {
      ...
      expect(viteConfig).toContain(`openbook({ framework: "${expectation.framework}" })`);
    });
  }
  ...
  it("overwrites preview when --force is set", async () => {
    ...
    expect(next).toContain(`from "openbook/react"`);
  });
  ...
  it("prints a snippet instead of overwriting vite.config.ts when one already exists", async () => {
    ...
    expect(output).toContain(`openbook({ framework: "svelte" })`);
  });
});
