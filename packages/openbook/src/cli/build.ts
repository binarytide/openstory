...
import { OpenbookBuildFailedError } from "../errors.js";
...
export const runBuild = async (projectRoot: string, options: BuildOptions): Promise<void> => {
  ...
  const manifest = await builder.build().catch((cause: unknown) => {
    throw new OpenbookBuildFailedError(cause instanceof Error ? cause.message : String(cause));
  });
  ...
  await mkdir(join(absoluteOutDir, "__openbook"), { recursive: true });
  await writeFile(
    join(absoluteOutDir, "__openbook", "manifest.json"),
  ...
  process.stdout.write(`built ${manifest.stories.length} stories to ${absoluteOutDir}\n`);
};
