import mri from "mri";
import { DEFAULT_DEV_PORT, OPENBOOK_VERSION } from "../constants.js";
import {
  OpenbookCliMissingArgError,
  OpenbookCliUnknownCommandError,
  OpenbookError,
} from "../errors.js";
...
export const run = async (argv: string[]): Promise<void> => {
  ...
  if (command === "--version" || command === "-v") {
    process.stdout.write(`openbook ${OPENBOOK_VERSION}\n`);
    return;
  }
  ...
  try {
    if (!isKnownCommand(command)) {
      throw new OpenbookCliUnknownCommandError(command, [...KNOWN_COMMANDS]);
    }
    ...
    switch (command) {
      ...
      case "inspect": {
        ...
        if (typeof storyId !== "string" || storyId === "") {
          throw new OpenbookCliMissingArgError("inspect", "id");
        }
    ...
    if (cause instanceof OpenbookError) {
