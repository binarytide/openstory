...
export interface BootOptions {
  id: string;
  exportName: string;
  renderer: OpenbookRenderer<unknown, unknown>;
...
const surfaceFatalMessage = (message: string): void => {
  const node = document.getElementById(OPENBOOK_ROOT_ELEMENT_ID) ?? document.body;
...
export const boot = (options: BootOptions): void => {
  const container = document.getElementById(OPENBOOK_ROOT_ELEMENT_ID);
  if (!container) {
    surfaceFatalMessage(`openbook: #${OPENBOOK_ROOT_ELEMENT_ID} not found in iframe document.`);
    return;
  }
  ...
  if (!story) {
    surfaceFatalMessage(`openbook: story "${escapeHtml(options.exportName)}" not found in module.`);
    return;
  }
