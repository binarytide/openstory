#!/usr/bin/env node
// Quality bar: every throw in packages/ode/src/ (excluding errors.ts) must use a typed OdeError class.
...
async function walk(dir) {
  ...
  for (const entry of entries) {
    ...
    lines.forEach((line, i) => {
      ...
      if (/throw\s+new\s+(Error|TypeError|RangeError|SyntaxError|EvalError|URIError|ReferenceError)\b/.test(line)) {
...
if (violations.length > 0) {
  console.error("Found throws that don't use a typed OdeError class:\n");
...
console.log("All throws in src/ use typed OdeError classes.");
