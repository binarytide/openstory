...
import {
  OdeCsfBadMetaError,
  OdeCsfDynamicTitleError,
  OdeCsfInvalidTagError,
  OdeCsfMissingDefaultExportError,
  OdeCsfParseError,
  OdeCsfStoriesOfError,
} from "../errors.js";
...
const coerceTags = (node: AstNode, filename: string): string[] => {
  ...
  if (!arrayNode || arrayNode.type !== "ArrayExpression") {
    throw new OdeCsfInvalidTagError(filename, "Expected tags array");
  }
  const tags: string[] = [];
  for (const element of at<Array<AstNode | null>>(arrayNode, "elements")) {
    ...
    if (typeof value !== "string") {
      throw new OdeCsfInvalidTagError(filename, "Expected tag to be string literal");
    }
    tags.push(value);
  }
  return tags;
...
const extractTitleFromMeta = (
  ...
  throw new OdeCsfDynamicTitleError(filename, getNodeLine(titleNode));
};
...
const collectNamedExports = (
  ...
  if (declaration.type === "VariableDeclaration") {
    for (const declarator of at<AstNode[]>(declaration, "declarations")) {
      ...
      exports.push(
        buildExportInfo(name, name, unwrapTypeAnnotations(at(declarator, "init"))),
      );
    }
    return;
  }
...
const scanProgram = (program: AstNode): ScannedProgram => {
  ...
  for (const statement of at<AstNode[]>(program, "body")) {
    ...
    if (
      ...
      isStoriesOfCall(at(statement, "expression"))
    ) {
      hasStoriesOf = true;
    }
  }
...
const extractStory = (meta: ParsedMeta, exportInfo: ExportInfo): ParsedStory => {
  ...
  const hasPlay =
    hasObjectProperty(storyObject, "play") || Boolean(annotations["play"]);
  const hasBeforeEach =
...
export const parseCsf = (source: string, options: ParseCsfOptions): ParsedCsf => {
  ...
  try {
    ...
    throw new OdeCsfParseError(filename, cause instanceof Error ? cause.message : String(cause));
  }
  if (parseResult.errors.length > 0) {
    throw new OdeCsfParseError(filename, parseResult.errors[0]!.message);
  }
  ...
  if (scanned.hasStoriesOf) {
    throw new OdeCsfStoriesOfError(filename);
  }
  ...
  if (!metaObject) {
    throw new OdeCsfMissingDefaultExportError(filename);
  }
  if (metaObject.type !== "ObjectExpression") {
    throw new OdeCsfBadMetaError(
      filename,
