...
import {
  OdeConfigAmbiguousFrameworkError,
  OdeConfigMissingFrameworkError,
} from "../errors.js";
...
export const detectFramework = async (projectRoot: string): Promise<Framework> => {
  ...
  if (candidates.length === 0) {
    throw new OdeConfigMissingFrameworkError(Object.keys(allDependencies));
  }
  if (candidates.length > 1) {
    throw new OdeConfigAmbiguousFrameworkError(candidates);
  }
  return candidates[0]!;
};
