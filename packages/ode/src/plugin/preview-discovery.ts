...
import {
  ...
  PREVIEW_FILE_LOCATIONS,
} from "../constants.js";
import {
  OdeConfigMultiplePreviewsError,
  OdeConfigPreviewNotFoundError,
} from "../errors.js";
...
export const findPreviewFile = async (projectRoot: string): Promise<string | undefined> => {
  ...
  if (matches.length > 1) {
    throw new OdeConfigMultiplePreviewsError(matches);
  }
  return matches[0];
};
...
export const requirePreviewFile = async (projectRoot: string): Promise<string> => {
  ...
  if (!path) {
    throw new OdeConfigPreviewNotFoundError(
