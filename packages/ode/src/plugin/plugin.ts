...
export const defineOde = (options: OdeOptions): OdeOptions => options;
...
const normalizeStories = (stories: OdeOptions["stories"]): string[] => {
...
const createStoryMiddleware =
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (!story) {
        throw new OdeStoryNotFoundError(storyId, [...storiesById.keys()]);
      }
...
export const ode = (userOptions: OdeOptions = {}): Plugin => {
  ...
  return {
    ...
    load: async (id) => {
      ...
      if (!story) {
        throw new OdeStoryNotFoundError(params.storyId, [...storiesById.keys()]);
      }
