...
import { ODE_VERSION } from "../constants.js";
import { OdeCsfDuplicateStoryIdError } from "../errors.js";
...
const deriveTitleFromImportPath = (
...
export class ManifestBuilder {
  ...
  build = async (): Promise<Manifest> => {
    ...
    for (const [id, paths] of importPathsById) {
      if (paths.length > 1) {
        throw new OdeCsfDuplicateStoryIdError(id, paths);
      }
    }
  ...
  private loadPreviewMetadata = async (): Promise<PreviewMetadata> => {
    ...
    try {
      ...
      this.devServer?.config.logger.warn(
        `[ode] preview parse failed for ${previewPath}: ${message}`,
      );
