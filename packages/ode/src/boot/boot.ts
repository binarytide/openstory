...
export interface BootOptions {
  id: string;
  exportName: string;
  renderer: OdeRenderer<unknown, unknown>;
...
const surfaceFatalMessage = (message: string): void => {
  const node = document.getElementById(ODE_ROOT_ELEMENT_ID) ?? document.body;
...
export const boot = (options: BootOptions): void => {
  const container = document.getElementById(ODE_ROOT_ELEMENT_ID);
  if (!container) {
    surfaceFatalMessage(`ode: #${ODE_ROOT_ELEMENT_ID} not found in iframe document.`);
    return;
  }
  ...
  if (!story) {
    surfaceFatalMessage(
      `ode: story "${escapeHtml(options.exportName)}" not found in module.`,
    );
    return;
  }
  ...
  const composedDecorators = composeDecorators(
    [
      ...(options.preview?.decorators ?? []),
      ...readDecorators(meta),
      ...readDecorators(story),
    ],
  ...
  const play =
    readFunction<PlayFunction>(story, "play") ??
    readFunction<PlayFunction>(meta, "play");
