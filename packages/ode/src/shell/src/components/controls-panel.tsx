...
const resolveControlType = (
  ...
  const stringControl =
    typeof definition.control === "string" ? definition.control : undefined;
...
const ControlInput = ({ argType, value, onChange }: ControlInputProps) => {
  ...
  return (
    <span className="text-xs italic text-muted-foreground">computed at runtime</span>
  );
};
      ...
      <div className="divide-y divide-border">
        {argTypes.map((argType) => {
          ...
          return (
            ...
            >
              <Label htmlFor={`arg-${argType.name}`} className="truncate text-xs text-muted-foreground">
                {argType.name}
              </Label>
              <div id={`arg-${argType.name}`}>
                <ControlInput
                  ...
                  value={currentValue}
                  onChange={(nextValue) =>
                    onChange({ ...args, [argType.name]: nextValue })
                  }
                />
              </div>
            </div>
