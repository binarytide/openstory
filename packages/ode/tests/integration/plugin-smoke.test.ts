// Phase 2 smoke test: boots a Vite dev server with ode() loaded against the
...
import { ode } from "../../src/plugin/index.js";
...
describe("plugin smoke: dev endpoints", () => {
  it("serves a valid manifest with preview metadata", async () => {
    const res = await fetch(`${baseUrl}/__ode/manifest.json`);
  ...
  it("serves story iframe HTML for a known story id", async () => {
    ...
    expect(html).toContain('data-ode-story="components-toolbar--default"');
    expect(html).toContain("ode-root");
    expect(html).toContain("window.__ODE_STORY__");
  ...
  it("returns 500 for an unknown story id", async () => {
    ...
    expect(body.error).toBe("OdeStoryNotFoundError");
  ...
  it("synthesizes a working virtual entry module via the html-proxy URL", async () => {
    ...
    // 2) Fetch the proxy script — it should contain a `virtual:ode-story-entry` import.
    const proxyRes = await fetch(`${baseUrl}${proxySrc}`);
    expect(proxyRes.status).toBe(200);
    const proxyCode = await proxyRes.text();
    expect(proxyCode).toMatch(/virtual:ode-story-entry/);

    // 3) Vite rewrites the bare `virtual:ode-story-entry` import to an internal
