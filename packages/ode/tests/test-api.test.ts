// Verifies ode/test re-exports the expected API surface for play functions.
...
// ode/test ships no .d.ts (the side-effect import of jest-dom matchers can't
// be DTS-bundled). Consumers get typings via jest-dom's ambient augmentation
// once they `import "ode/test"`; tests dynamic-import as `unknown`.
async function loadOdeTest(): Promise<Record<string, unknown>> {
  // @ts-expect-error — see comment above; ode/test has no DTS by design.
  const mod = (await import("ode/test")) as Record<string, unknown>;
  return mod;
}

describe("ode/test API surface", () => {
  it("exports the full play-function toolkit", async () => {
    const api = await loadOdeTest();
