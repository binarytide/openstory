...
export interface OpenstoryUiOptions {
  title?: string;
...
const createStoryMiddleware =
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (!story) {
        throw new OpenstoryStoryNotFoundError(storyId, [...storiesById.keys()]);
      }
...
export const openstory = (userOptions: OpenstoryOptions = {}): Plugin => {
