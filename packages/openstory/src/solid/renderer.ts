...
const renderInto = (
  ...
  render(() => options.render(options.args, options.context) as unknown as Element, container);

export const renderer: OpenstoryRenderer<unknown, SolidMounted> = {
  mount: (options) => {
  ...
  update: (mounted, options) => {
    mounted.dispose();
    mounted.dispose = renderInto({ ...options, container: mounted.container }, mounted.container);
  },
  unmount: (mounted) => {
    mounted.dispose();
  },
