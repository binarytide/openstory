import { OpenstoryCsfInvalidStoryNameError, OpenstoryCsfInvalidTitleError } from "../errors.js";
...
const sanitizeTitle = (input: string): string => {
  ...
  if (sanitized === "") {
    throw new OpenstoryCsfInvalidTitleError("<inline>", input);
  }
  return sanitized;
};
...
const sanitizeStoryName = (input: string): string => {
  const sanitized = sanitize(input);
  if (sanitized === "") {
    throw new OpenstoryCsfInvalidStoryNameError(input);
  }
  return sanitized;
};
