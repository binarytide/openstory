...
import {
  OpenstoryCsfBadMetaError,
  OpenstoryCsfDynamicTitleError,
  OpenstoryCsfInvalidTagError,
  OpenstoryCsfMissingDefaultExportError,
  OpenstoryCsfParseError,
  OpenstoryCsfStoriesOfError,
} from "../errors.js";
...
const coerceTags = (node: AstNode, filename: string): string[] => {
  ...
  if (!arrayNode || arrayNode.type !== "ArrayExpression") {
    throw new OpenstoryCsfInvalidTagError(filename, "Expected tags array");
  }
  const tags: string[] = [];
  for (const element of at<Array<AstNode | null>>(arrayNode, "elements")) {
    ...
    if (typeof value !== "string") {
      throw new OpenstoryCsfInvalidTagError(filename, "Expected tag to be string literal");
    }
    tags.push(value);
  }
  return tags;
...
const extractTitleFromMeta = (metaObject: AstNode, filename: string): string | undefined => {
  ...
  throw new OpenstoryCsfDynamicTitleError(filename, getNodeLine(titleNode));
};
...
export const parseCsf = (source: string, options: ParseCsfOptions): ParsedCsf => {
  ...
  if (scanned.hasStoriesOf) {
    throw new OpenstoryCsfStoriesOfError(filename);
  }
  ...
  if (!metaObject) {
    throw new OpenstoryCsfMissingDefaultExportError(filename);
  }
