// Phase 2 smoke test: boots a Vite dev server with openstory() loaded against the
...
describe("plugin smoke: dev endpoints", () => {
  it("serves a valid manifest with preview metadata", async () => {
    const res = await fetch(`${baseUrl}/__openstory/manifest.json`);
  ...
  it("serves story iframe HTML for a known story id", async () => {
    ...
    expect(html).toContain('data-openstory-story="components-toolbar--default"');
    expect(html).toContain("openstory-root");
    expect(html).toContain("window.__OPENSTORY_STORY__");
  ...
  it("returns 500 for an unknown story id", async () => {
    ...
    expect(body.error).toBe("OpenstoryStoryNotFoundError");
  ...
  it("synthesizes a working virtual entry module via the html-proxy URL", async () => {
    ...
    // 2) Fetch the proxy script. It should contain a `virtual:openstory-story-entry` import.
    const proxyRes = await fetch(`${baseUrl}${proxySrc}`);
    expect(proxyRes.status).toBe(200);
    const proxyCode = await proxyRes.text();
    expect(proxyCode).toMatch(/virtual:openstory-story-entry/);

    // 3) Vite rewrites the bare `virtual:openstory-story-entry` import to an internal
